    ...
    return (
        ...
        <div className="w-full min-w-0 py-0.5 break-words text-metal-100">
          ...
          <div className="flex justify-end items-center flex-wrap gap-2 mt-2 pt-2 border-t border-white/[0.06]">
                {canPlayAudio && (<button onClick={() => onPlayAudio(message.id, textContent)} className={`tap flex items-center justify-center rounded-full transition-colors ${isPlaying ? 'text-metal-100 bg-raised' : 'text-metal-300 md:hover:bg-metal-700 md:hover:text-metal-100'}`} aria-label={isPlaying ? "Stop" : "Read aloud"}>{isPlaying ? <StopIcon /> : <PlayIcon />}</button>)}
