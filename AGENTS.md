...
- `memory/bridge.js` / `memory/routes.js` — the server's side: one `GraphMemory` for the whole builder (`.multi-memory/builder.db`, project id `builder`; the build id is an attribution dimension, not a file name), and the `/api/memory/*` surface.
