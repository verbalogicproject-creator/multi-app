    ...
    return (
        <div className="p-4 rounded-2xl max-w-lg lg:max-w-2xl xl:max-w-4xl break-words bg-gray-700 rounded-bl-none">
          ...
          <div className="flex justify-end items-center flex-wrap gap-2 mt-2 pt-2 border-t border-gray-600/50">
                {canPlayAudio && (
                    <button 
                        onClick={() => onPlayAudio(message.id, textContent)}
                        className={`p-1.5 rounded-full transition-colors ${isPlaying ? 'text-white bg-sky-600' : 'text-gray-300 hover:bg-gray-600 hover:text-white'}`}
                        aria-label={isPlaying ? "Stop reading aloud" : "Read message aloud"}
                    >
                        {isPlaying ? <StopIcon /> : <PlayIcon />}
                    </button>
                )}
                {message.parts.some(p => p.imageUrl) && onUseImage && (
                    <button
                        onClick={() => onUseImage(message.parts.find(p => p.imageUrl)!.imageUrl!)}
                        className="px-3 py-1 bg-gray-600 text-gray-200 text-xs font-semibold rounded-md hover:bg-sky-600 transition-colors flex items-center justify-center"
                        aria-label="Use this image for next edit"
                    >
                       <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}> <path strokeLinecap="round" strokeLinejoin="round" d="M4 4v5h5M5 5h14a2 2 0 012 2v10a2 2 0 01-2 2H5a2 2 0 01-2-2V7a2 2 0 012-2z" /> </svg>
                        Use this image
                    </button>
                )}
                {canContinueScene && (
                    <button
                        onClick={() => onContinueScene(message.id)}
                        className="px-3 py-1 bg-gray-600 text-gray-200 text-xs font-semibold rounded-md hover:bg-sky-600 transition-colors flex items-center justify-center"
                        aria-label="Continue video scene"
                    >
          ...
          {message.toolCall && (
            <div className="mt-2 p-3 bg-gray-800/50 border border-gray-600 rounded-lg text-sm text-gray-300">
                <div className="flex items-center gap-2 font-mono text-xs text-sky-400">
                    ...
                    <span>Tool Call: {message.toolCall.name}</span>
                </div>
                <pre className="mt-1 p-2 bg-gray-900 rounded text-xs overflow-x-auto">
                    <code>{JSON.stringify(message.toolCall.args, null, 2)}</code>
                </pre>
            </div>
          )}
          {message.groundingMetadata && message.groundingMetadata.length > 0 && (
            <div className="mt-4 pt-3 border-t border-gray-600">
              ...
              <ul className="space-y-1">
                {message.groundingMetadata.map((chunk, index) => (
